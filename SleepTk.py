...
_FREQ = const(60)  # get accelerometer data every X seconds, but process and store them only every _STORE_FREQ seconds
_STORE_FREQ = const(300)  # process data and store to file every X seconds
_BATTERY_THRESHOLD = const(-500)  # under X% of battery, stop tracking and only keep the alarm
...
_CYCLE_LENGTH = const(100)  # in minutes, default of 90 or 100, according to https://sleepyti.me/  # currently used only to display best wake up time, not to compute smart alarm!
